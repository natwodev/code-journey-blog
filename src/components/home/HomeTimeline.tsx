          ...
          {timeline.map((item, idx) => {
            ...
            return (
              <motion.div
                ...
                style={{ marginLeft: isDesktop ? step : 0 }}
              >
...
function TimelineStepCard({
  ...
  return (
    <motion.button
      ...
      className="group relative w-full text-left rounded-2xl p-4 sm:p-6 bg-white/6 border border-white/10 backdrop-blur-sm hover:bg-white/8 transition max-w-full md:max-w-2xl"
    >
      {/* dot for mobile left */}
      ...
      <div className="flex items-start gap-3 sm:gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-brand-cyan/90 text-xs sm:text-sm">Mốc: {year}</div>
          ...
          <Link to={ctaHref} className="mt-4 inline-block px-3 py-2 sm:px-4 rounded-lg border border-white/15 hover:bg-white/5 transition text-sm">Xem chi tiết</Link>
        </div>
      </div>
