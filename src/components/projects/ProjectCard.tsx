...
export default function ProjectCard({ project }: { readonly project: Project }) {
  ...
  return (
    <Link to={`/projects/${project.id}`} ref={ref as unknown as React.Ref<HTMLAnchorElement>} onMouseMove={onMouseMove as unknown as React.MouseEventHandler<HTMLAnchorElement>} onMouseLeave={onMouseLeave as unknown as React.MouseEventHandler<HTMLAnchorElement>} className="group block text-left rounded-2xl bg-white/5 hover:bg-white/8 transition neon-border will-change-transform hover:-translate-y-1 shadow-[0_0_0_rgba(0,201,167,0)] hover:shadow-[0_0_28px_rgba(0,201,167,0.22)] overflow-hidden">
